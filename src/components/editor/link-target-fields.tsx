...
export function LinkTargetFields({
  ...
  useEffect(() => {
    if (!siteId || (mode !== "collection" && mode !== "url")) return;
  ...
  useEffect(() => {
    if (mode === "collection" && !collectionSlug && collections[0]?.slug) {
      onUpdateProp("linkCollectionSlug", collections[0].slug);
    }
  }, [mode, collectionSlug, collections, onUpdateProp]);

  useEffect(() => {
    if (!siteId || mode !== "collection" || !collectionSlug) {
      setItems([]);
      return;
    }
  ...
  }, [siteId, mode, collectionSlug, collections]);
