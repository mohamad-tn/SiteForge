...
export function HomeLanding() {
  ...
  return (
    <AppCanvas dir={dir} lang={lang}>
      <header className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-6 sm:px-6">
      ...
      <main className="mx-auto max-w-6xl px-4 py-16 sm:px-6 md:py-24">
        <div className="max-w-3xl">
          <p className="mb-4 inline-flex rounded-full border border-teal-800/10 bg-[var(--card)]/80 px-3 py-1 text-xs font-semibold text-teal-800 shadow-[var(--shadow-xs)] backdrop-blur dark:border-teal-400/20 dark:text-teal-300">
            {t("homeBadge")}
          </p>
          <h1 className="mb-6 text-4xl font-bold leading-[1.15] tracking-tight md:text-6xl">{t("homeTitle")}</h1>
          <p className="mb-10 text-lg leading-8 text-[var(--muted)]">{t("homeBody")}</p>
        ...
        <div className="mt-16 grid gap-4 md:grid-cols-3">
          {features.map(([title, desc]) => (
            <SoftCard key={title} className="p-6">
